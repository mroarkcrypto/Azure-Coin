...
SpaceXpanse inherits these interfaces.  However, SpaceXpanse focuses on providing
...
purpose**, tailored specifically for the SpaceXpanse [game model](games.md).
...
current game state updated with the SpaceXpanse blockchain.  For this, it must
process moves from attached and detached blocks as discussed in the
SpaceXpanse [game model](games.md).

### Attaching and Detaching Publishers <a name="attach-detach"></a>

The SpaceXpanse daemon provides [ZeroMQ publishers](http://zeromq.org/) for
...
* **`TXID`:**
  The SpaceXpanse transaction ID of the transaction that performed the given move.
...
* **`ADDRESS`n and `AMOUNT`n:**
  SpaceXpanse addresses and amounts that were transacted in the move transaction,
  as described in the model for
  [currency transaction in games](games.md#currency).
* **`BURNT`:**
  The total amount of ROD burnt in this transaction with data `g/GAMEID`
...
Note that SpaceXpanse typically allows a single name to be updated only once per block,
...
corresponds to the tip of the current SpaceXpanse blockchain.  In this case,
...
with the SpaceXpanse blockchain in real time.
...
out-of-sync with the SpaceXpanse daemon.  This could be because the game engine was
...
for a new game is installed and attached to the SpaceXpanse daemon for the first time
...
If the SpaceXpanse daemon knows both block hashes and there is a sequence of block
...
(e.g. when a game is synced from scratch), SpaceXpanse Core may decide to send only
...
The SpaceXpanse daemon can partially optimise this process itself:
...
The SpaceXpanse daemon's interface provides two complementary methods for this.
...
* **`TXID`**:
  The SpaceXpanse transaction ID of the name update.
* **`NAME`**:
