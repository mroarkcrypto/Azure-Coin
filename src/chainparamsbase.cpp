...
/**
 * Port numbers for incoming Tor connections (11999, 18399, 38399, 18499) have
 * been chosen arbitrarily to keep ranges of used ports tight.
 */
std::unique_ptr<CBaseChainParams> CreateBaseChainParams(const std::string& chain)
{
    if (chain == CBaseChainParams::MAIN) {
        // Azure Guardian mainnet RPC / onion (must not collide with LRGK 53685)
        return std::make_unique<CBaseChainParams>("", 49825, 49826);
    } else if (chain == CBaseChainParams::TESTNET) {
        return std::make_unique<CBaseChainParams>("testnet3", 18398, 18399);
    } else if (chain == CBaseChainParams::SIGNET) {
        return std::make_unique<CBaseChainParams>("signet", 38398, 38399);
    } else if (chain == CBaseChainParams::REGTEST) {
        return std::make_unique<CBaseChainParams>("regtest", 18498, 18499);
    }
