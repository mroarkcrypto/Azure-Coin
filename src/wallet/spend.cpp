        ...
        bool sign)
{
    ...
    // Limit size (standardness). Prefer largest UTXOs; if still over limit the
    // wallet holds too many dust/mining fragments — consolidate first.
    if ((sign && GetTransactionWeight(*tx) > MAX_STANDARD_TX_WEIGHT) ||
        (!sign && tx_sizes.weight > MAX_STANDARD_TX_WEIGHT))
    {
        error = _("Transaction too large. Your balance is split across too many small outputs (common after mining). Use Coin Control to spend larger UTXOs first, or send/consolidate in smaller batches.");
        return false;
    }
